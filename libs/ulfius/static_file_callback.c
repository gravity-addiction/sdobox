...
printf("Looking For: %s\n", file_path);
    if (access(file_path, F_OK) != -1) {
      ...
      if (f) {
        ...
        if (content_type == NULL) {
          ...
          dbgprintf(DBG_INFO, "Static File Server - Unknown mime type for extension %s", get_filename_ext(file_requested));
        }
        ...
        if (ulfius_set_stream_response(response, 200, callback_static_file_stream, callback_static_file_stream_free, length, STATIC_FILE_CHUNK, f) != U_OK) {
          dbgprintf(DBG_ERROR, "callback_static_file - Error ulfius_set_stream_response");
        }
      }
    } else {
    ...
    dbgprintf(DBG_ERROR, "Static File Server - Error, user_data is NULL or inconsistent");
